...
internal object QushayriTafsirRepository {
    private val ASSET_PARTS = (0..6).map { index ->
        "tafsir/al_qushayri_suras_1_4_en.sqlite.gz.b64.part%02d".format(index)
    }
    private const val DATABASE_NAME = "al_qushayri_suras_1_4_en.sqlite"
    private const val EXPECTED_ENTRIES = 669
    private const val EXPECTED_SHA256 =
        "b398df3c2c55bea7e8a529d64c4672686ec0d5bc416e74ddbf3359a1ee5e9b2f"

    fun covers(verse: VerseRef): Boolean = verse.surah in 1..4

    suspend fun load(context: Context, verse: VerseRef): TafsirEntry? =
        withContext(Dispatchers.IO) {
            if (!covers(verse)) return@withContext null
            ...
            try {
                ...
                TafsirEntry(verse, commentaryRuns, emptyList())
            } finally {
                database.close()
            }
        }
    ...
    private fun materializeDatabase(context: Context): File? {
        ...
        return runCatching {
            ...
            check(sha256(temporary) == EXPECTED_SHA256) {
                "Qushayri tafsir database integrity check failed"
            }
            if (destination.exists() && !destination.delete()) {
                error("Cannot replace the previous Qushayri tafsir database")
            }
            check(temporary.renameTo(destination)) {
                "Cannot install the Qushayri tafsir database atomically"
            }
            destination
        }.getOrNull().also {
    ...
    private fun openDatabaseArchive(context: Context): GZIPInputStream {
        ...
        return try {
            ...
            val joined = SequenceInputStream(Collections.enumeration(streams))
            GZIPInputStream(Base64InputStream(joined, Base64.DEFAULT))
