...
private const val MULTI_FONT_KEY = "commentary_font_size_sp"
private const val MIN_TAFSIR_FONT = 14f
private const val MAX_TAFSIR_FONT = 26f
private const val DEFAULT_TAFSIR_FONT = 18f
...
internal fun MultiTafsirPanel(
    ...
    val prefs = remember {
        context.getSharedPreferences(MULTI_TAFSIR_PREFS, android.content.Context.MODE_PRIVATE)
    }
    var selectedEdition by remember {
        mutableStateOf(
            PrivateTafsirEdition.fromStorage(
                prefs.getString(MULTI_EDITION_KEY, PrivateTafsirEdition.JALALAYN.storageValue)
            )
        )
    }
    ...
    var fontSize by remember {
        mutableFloatStateOf(
            prefs.getFloat(MULTI_FONT_KEY, DEFAULT_TAFSIR_FONT)
                .coerceIn(MIN_TAFSIR_FONT, MAX_TAFSIR_FONT)
        )
    }
    ...
    LaunchedEffect(requestKey) {
        ...
        val entry = MultiTafsirRepository.load(context, verse, selectedEdition)
        if (activeRequest == requestKey) {
            state = if (entry == null) TafsirLoadState.Unavailable else TafsirLoadState.Available(entry)
        }
    }
    ...
    fun choose(edition: PrivateTafsirEdition) {
        ...
        prefs.edit().putString(MULTI_EDITION_KEY, edition.storageValue).apply()
        selectedEdition = edition
    }

    fun changeFont(delta: Float) {
        val next = (fontSize + delta).coerceIn(MIN_TAFSIR_FONT, MAX_TAFSIR_FONT)
        fontSize = next
        prefs.edit().putFloat(MULTI_FONT_KEY, next).apply()
    }
    ...
    Surface(
        ...
        Column(
            ...
            Row(
                ...
                Row {
                    TextButton(
                        modifier = Modifier.semantics { contentDescription = "Réduire la taille du commentaire" },
                        enabled = fontSize > MIN_TAFSIR_FONT,
                        onClick = { changeFont(-2f) }
                    ) { Text("A−") }
                    TextButton(
                        modifier = Modifier.semantics { contentDescription = "Agrandir la taille du commentaire" },
                        enabled = fontSize < MAX_TAFSIR_FONT,
                        onClick = { changeFont(2f) }
                    ) { Text("A+") }
                }
            }
            ...
            when (val current = state) {
                ...
                is TafsirLoadState.Available -> {
                    Text(
                        text = multiRunsToAnnotatedString(current.entry.commentaryRuns),
                        fontSize = fontSize.sp,
                        lineHeight = (fontSize * 1.42f).sp,
                        color = MaterialTheme.colorScheme.onSurface
                    )
                    if (current.entry.notes.isNotEmpty()) {
                        HorizontalDivider()
                        TextButton(
                            modifier = Modifier.semantics {
                                stateDescription = if (notesExpanded) "Notes développées" else "Notes repliées"
                            },
                            onClick = { notesExpanded = !notesExpanded }
                        ) {
                            Text(
                                if (notesExpanded) {
                                    "Notes (${current.entry.notes.size}) — replier"
                                } else {
                                    "Notes (${current.entry.notes.size})"
                                }
                            )
                        }
                        if (notesExpanded) {
                            current.entry.notes.forEach { note ->
                                Text(
                                    text = AnnotatedString.Builder().apply {
                                        withStyle(SpanStyle(fontWeight = FontWeight.Bold)) {
                                            append("${note.number}. ")
                                        }
                                        append(multiRunsToAnnotatedString(note.runs))
                                    }.toAnnotatedString(),
                                    fontSize = (fontSize - 1f).coerceAtLeast(MIN_TAFSIR_FONT).sp,
...
private fun multiRunsToAnnotatedString(runs: List<TafsirRun>): AnnotatedString =
