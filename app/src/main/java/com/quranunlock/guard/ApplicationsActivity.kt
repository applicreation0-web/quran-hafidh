...
class ApplicationsActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        AppCatalog.refresh()
        setContent { QuranSafeguardTheme { ApplicationsScreen() } }
    }
    ...
    private fun ApplicationsScreen() {
        ...
        val pendingRemovals = remember {
            mutableStateListOf<String>().apply { addAll(initialSelection.second.sorted()) }
        }
        var removalRequest by remember { mutableStateOf<Set<String>>(emptySet()) }
        ...
        if (removalRequest.isNotEmpty()) {
            ...
            AlertDialog(
                ...
                title = {
                    Text(
                        if (removalRequest.size == 1) "Retirer cette application demain ?"
                        else "Retirer ces applications demain ?"
                    )
                },
                text = {
                    Text(
                        buildString {
                            ...
                            append(
                                "La protection reste active aujourd’hui. Le retrait prendra effet au prochain jour et vous pourrez l’annuler d’ici là en recochant."
                            )
                        }
                    )
                },
                confirmButton = {
                    TextButton(
                        onClick = {
                            ...
                            GuardPrefs.saveProtectedPackages(this@ApplicationsActivity, selected.toSet())
        ...
        Column(
                    ...
                    Text(
                        if (pendingRemovals.isNotEmpty()) {
                            pendingRemovals.size.toString() +
                                if (pendingRemovals.size == 1) " retrait prévu demain" else " retraits prévus demain"
                        } else {
                            (installedTargets.size - selected.size).coerceAtLeast(0).toString() + " non sélectionnées"
                        },
            ...
            Column(
                ...
                SafeguardOutlinedButton(
                    modifier = Modifier.fillMaxWidth(),
                    onClick = { removalRequest = (selected + pendingRemovals).toSet() }
            ...
            TargetGroup(
                ...
                targets = installedTargets.filter { it.category == SafeguardTargetCategory.SOCIAL },
                selected = selected,
                pendingRemovals = pendingRemovals.toSet(),
                onToggle = { target, checked ->
                    if (checked) {
                        if (target.packageName !in selected) selected.add(target.packageName)
                        GuardPrefs.saveProtectedPackages(this@ApplicationsActivity, selected.toSet())
                        reloadSelection()
                    } else removalRequest = setOf(target.packageName)
                }
            )
            ...
            TargetGroup(
                ...
                targets = installedTargets.filter { it.category == SafeguardTargetCategory.BROWSER },
                selected = selected,
                pendingRemovals = pendingRemovals.toSet(),
                onToggle = { target, checked ->
                    if (checked) {
    ...
    ElevatedCard(
        ...
        Column(modifier = Modifier.padding(horizontal = 16.dp, vertical = 16.dp)) {
            ...
            targets.forEachIndexed { index, target ->
                if (index == 0) HorizontalDivider(modifier = Modifier.padding(top = 12.dp, bottom = 4.dp))
                Row(
                    ...
                    Column(modifier = Modifier.weight(1f)) {
                        ...
                        Text(
                            when {
                                target.packageName in pendingRemovals -> "Retrait prévu demain · cochez pour annuler"
                                target.category == SafeguardTargetCategory.BROWSER -> "Navigateur"
                                else -> "Réseau / messagerie"
                            },
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant
                        )
