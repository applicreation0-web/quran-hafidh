...
public final class MainActivity extends android.app.Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        TextView directTitle=Ui.text(this,"Accès direct au parcours",16,true);
        directTitle.setPadding(0,Ui.dp(this,18),0,Ui.dp(this,4));root.addView(directTitle);
        TextView directHelp=Ui.text(this,"Ouvrir un mode sans changer le calendrier. L’ouverture seule ne déplace aucun curseur.",13,false);
    ...
    private String personalDayPlan(DayOfWeek day) {
        switch (day) {
            case MONDAY:
                return "Matin · Sabqi · 5 lignes · 90 min\nSoir · Révision Sabqi · 15–20 min";
            case TUESDAY:
    ...
    private void refreshToday(){
        ...
        String detail;
        switch(kind){
            case SABQI:{int cursor=prefs.sabqiLineCursor();if(cursor<0)cursor=localGeometry.firstLineIndex(new VerseRef(2,75));GeometryRepository.FiveLineBlock b=localGeometry.fiveLineBlock(cursor);detail="Séance structurée actuelle · Sabqi · "+b.verseLabel();break;}
            case ITQAN:{
                ...
                else {GeometryRepository.VerseUnit u=localGeometry.eligiblePageUnit(prefs.itqanCursor(),prefs.corpus());detail="Séance structurée actuelle · Itqān · "+u.start+" → "+u.end;}
                break;
            }
