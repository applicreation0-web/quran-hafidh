#!/usr/bin/env python3
"""Set exact Android version metadata and release-audit tuple for Quran Safeguard 0.10.8."""
...
text = text[:match.start()] + match.group(1) + "27" + match.group(2) + "0.10.8" + match.group(3) + text[match.end():]

# Keep the audited 0.10.3 source baseline accepted while making 0.10.8 the only prepared release tuple.
...
if text.count('versionCode = 27') != 2:
    raise SystemExit("0.10.8 versionCode 27 must appear exactly in defaultConfig and release audit")
if text.count('versionName = "0.10.8"') != 1:
    raise SystemExit("0.10.8 defaultConfig versionName must appear exactly once")
if text.count('versionName = \\\"0.10.8\\\"') != 1:
    raise SystemExit("0.10.8 escaped release-audit versionName must appear exactly once")

path.write_text(text, encoding="utf-8")
print("Prepared Quran Safeguard 0.10.8: versionCode 27 / Plus suffix -plus.1 / exact release-audit tuple")
