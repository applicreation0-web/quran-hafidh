...
req('EXPECTED_PARTS' in asset_prep and 'prepare_package' in asset_prep and 'publish_packages' in asset_prep,'asset preparation must verify part counts and stage both corpora before publication')
req(asset_prep.index("payloads={") < asset_prep.index('publish_packages(payloads,assets,work)'),'asset preparation must validate both payloads before publishing either corpus')
...
print('- both corpora are fully staged and verified before final Plus assets are replaced')
