...
def between(text, start, end, new, label):
    a = text.find(start)
    if a < 0:
        raise SystemExit(f'{label}: start marker absent')
    b = text.find(end, a)
    if b < 0:
        raise SystemExit(f'{label}: end marker absent')
...
'''
s = rep(s, old_repair, new_repair, 'schema4 repair')
s = rep(s,
...
new_complete = '''    /** Atomically advances the natural Itqan cycle and consolidates the exact completed queue entry. */
...
queue_block = '''    public List<AnchoringQueue.Entry> anchoringQueue() {
...
s = between(s,
    '    public List<AnchoringQueue.Entry> anchoringQueue()',
    '    public String sabqiTodayReviewDate()',
    queue_block,
    'anchoring queue block')

old_recent_complete = '''    public boolean completeRecentSabqiReview(String date, int nextIndex, String label) {
        return p.edit()
...
'''
new_recent_complete = '''    public boolean completeRecentSabqiReview(String date, int nextIndex, String label) {
...
'''
s = rep(s, old_recent_complete, new_recent_complete, 'Consolidation attendance commit')
s = rep(s,
...
'''
s = between(s,
    '    /**\n     * Idempotently grows the Itqan snowball.',
...
s = rep(s,
    ...
    '''        List<LocalDate> completed = activation == null
            ? Collections.emptyList()
            : prefs.consolidationAttendanceDates(plannedStart, today);''',
    'promotion status durable attendance')
s = rep(s,
...
s = rep(s,
    ...
    'forced completed Ancrage header')
s = rep(s,
    '        program.setText("Ancrage · "+itqanUnit.start+" → "+itqanUnit.end+" · ×"+itqanTargetReps);',
    '        program.setText("Ancrage · "+itqanUnit.start+" → "+itqanUnit.end+" · ×"+itqanTargetReps\n            +(anchoringEntry.origin==AnchoringQueue.Origin.FORCED_PROMOTION?" · promotion de sécurité":""));',
    'forced Ancrage header')
s = rep(s,
    '''        VerseRef itqanBefore = prefs.itqanCursor();
        EligibleCorpus corpus = prefs.murajaahCorpus();''',
    '        EligibleCorpus corpus = prefs.murajaahCorpus();',
    'remove unreachable cursor snapshot')
s = rep(s,
    ...
    '''        if (!ok) { onError("Impossible d’enregistrer la validation de l’Entretien."); return; }
        closeClockForCompletedSession();''',
    'remove unreachable cursor restore')
path.write_text(s)

# MainActivity
path = Path('hifz-app/src/main/java/com/quransafeguard/hifz/preview/MainActivity.java')
s = path.read_text()
s = rep(s,
