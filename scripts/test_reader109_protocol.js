...
for(const n of [1,2,3,4,5,6,7,8,10,11,12,15,16,31]){
 ...
 const sizes=P.balancedBlocks(lines).map(b=>b.length);assert.deepEqual(P.balancedBlocks(lines).flat(),lines);assert(sizes.every(size=>size<=6));if(sizes.length>1){assert(sizes.every(size=>size>=3));assert(Math.max(...sizes)-Math.min(...sizes)<=1)}if(n===7)assert.deepEqual(sizes,[4,3]);if(n===11)assert.deepEqual(sizes,[6,5]);
