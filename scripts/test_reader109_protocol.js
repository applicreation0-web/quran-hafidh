...
const assisted=P.create(['1:0'],false);while(P.current(assisted).mask!==100){const p=P.current(assisted);for(let i=0;i<p.min;i++)P.record(assisted,true);assert(P.validate(assisted))}const assistedStep=P.current(assisted);assisted.counts[assistedStep.id]=7;assisted.wins[assistedStep.id]=2;P.aid(assisted,'first-word',1);assert.equal(assisted.wins[assistedStep.id],0);assert(!P.record(assisted,true));assert(!P.canValidate(assisted));P.clearAid(assisted);assert(P.record(assisted,true));assert.equal(assisted.wins[assistedStep.id],1);assert(!P.canValidate(assisted));P.aid(assisted,'brief',0);assert(!P.record(assisted,true));P.redo(assisted);assert.equal(assisted.assistance,undefined);assert.equal(assisted.counts[assistedStep.id],0);assert.equal(assisted.wins[assistedStep.id],0);
